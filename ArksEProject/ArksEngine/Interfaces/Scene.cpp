...
void ArksEngine::Scene::Update(double deltaTime) const
{
    ...
    if (!m_pGameObjectManager->GetGameObject("RedWanderer")->GetComponent<ArksEngineComponents::HealthComponent>().GetIsAlive())
    {
        m_pGameObjectManager->GetGameObject("RedWanderer")->Destroy();
    	m_pGameObjectManager->Refresh();

        m_pOwner->SetQuit(true);
        m_pOwner->GetDebug_Log()->Log("You Died");
    }
